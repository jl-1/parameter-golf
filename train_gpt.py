...
class Hyperparameters:
    ...
    eval_stride = int(os.environ.get("EVAL_STRIDE", 0))  # 0 = no sliding window; >0 = stride for sliding window eval
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor], skip_quantize_prefixes: tuple[str, ...] = ()):
    # Single supported clean-script export format:
    # - per-row int8 for 2D float tensors
    # - per-tensor int8 for other float tensors
    # - exact passthrough for non-floats
    # - passthrough for small float tensors, stored as fp16 to save bytes
    # - skip_quantize_prefixes: store these as fp16 passthrough (avoid quant error compounding)
    ...
    for name, tensor in state_dict.items():
        ...
        # Keep shared body kernel weights at fp16 to avoid quantization error compounding
        if skip_quantize_prefixes and name.startswith(skip_quantize_prefixes):
...
class BodyControl(nn.Module):
    # Per-application control tensors for a shared body block.
...
class GPT(nn.Module):
    def __init__(
        ...
        def _make_block(include_controls: bool = True) -> Block:
            return Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, include_controls)
        ...
        if share_body:
            ...
            if num_body_kernels < 1:
                raise ValueError(f"num_body_kernels must be >= 1, got {num_body_kernels}")
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.share_body:
            ...
            for block in self.stem_blocks:
                x = block(x, x0)
                skips.append(x)
            x_stem = x  # final stem output as residual reference for body
            # Body: alternating shared kernels with per-application controls.
            # Stem residual connects to every other body layer (1st, 3rd, ...).
...
def main() -> None:
    ...
    # EMA: maintain exponential moving average of model weights
        ...
        if ema_state is not None:
            ...
            with torch.no_grad():
                for n, p in base_model.named_parameters():
                    ema_state[n].lerp_(p.data, 1.0 - decay)
    ...
    if ema_state is not None:
        log0("Using EMA weights for final eval and serialization")
        for n, p in base_model.named_parameters():
    ...
    skip_prefixes = ("body_kernels.",) if args.share_body else ()
    quant_obj, quant_stats = quantize_state_dict_int8(base_model.state_dict(), skip_quantize_prefixes=skip_prefixes)
    ...
    if master_process:
        with open("final_model.int8.ptzst", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = os.path.getsize("final_model.int8.ptzst")
        code_bytes = len(code.encode("utf-8"))
    ...
    if distributed:
        dist.barrier()
    with open("final_model.int8.ptzst", "rb") as f:
    ...
    torch.cuda.synchronize()
    sw_label = f"stride={args.eval_stride}" if args.eval_stride > 0 else "no_sw"
    log0(
        f"final_int8_zstd_roundtrip({sw_label}) val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
