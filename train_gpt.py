...
class GPT(nn.Module):
    def __init__(
        ...
        # Shared body: reuse bank entries for body layers, keep unique controls per block.
        self.share_body = share_body
        self._num_stem = num_stem_layers
        self._num_head = num_head_layers
        self._num_body_kernels = num_body_kernels
        # Number of unique bank entries: all layers if no sharing, else stem + kernels + head.
        nb = (num_stem_layers + num_body_kernels + num_head_layers) if share_body else num_layers
        self._num_bank = nb
        ...
        self.qo_bank = nn.Parameter(torch.empty(2 * nb, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * nb, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(nb, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(nb, model_dim, mlp_dim))
        ...
        self._init_weights()
    def _bank_idx(self, layer_idx: int) -> int:
        """Map logical layer index to bank row index."""
        if not self.share_body:
            return layer_idx
        if layer_idx < self._num_stem:
            return layer_idx
        if layer_idx >= self.num_layers - self._num_head:
            return self._num_stem + self._num_body_kernels + (layer_idx - (self.num_layers - self._num_head))
        return self._num_stem + (layer_idx - self._num_stem) % self._num_body_kernels
    def _init_weights(self) -> None:
        ...
        for i in range(nb):
            ...
            nn.init.zeros_(self.qo_bank.data[nb + i])                   # Out (zero init)
            nn.init.orthogonal_(self.kv_bank.data[i], gain=1.0)        # K
            nn.init.orthogonal_(self.kv_bank.data[nb + i], gain=1.0)   # V
            ...
            self.qo_bank.data[nb + i].mul_(proj_scale)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[b], self.kv_bank[b], self.kv_bank[nb + b],
                self.qo_bank[nb + b], self.mlp_up_bank[b], self.mlp_down_bank[b],
                v_embed=ve, v0=v0)
...
def main() -> None:
    ...
    if args.share_body:
        ...
        bank_params = sum(p.numel() for p in [base_model.qo_bank, base_model.kv_bank, base_model.mlp_up_bank, base_model.mlp_down_bank])
        log0(
            f"shared_body: stem={args.num_stem_layers} body_apps={num_body_apps} "
            f"kernels={args.num_body_kernels} head={args.num_head_layers} "
            f"bank_entries={base_model._num_bank} bank_params={bank_params}"
        )
