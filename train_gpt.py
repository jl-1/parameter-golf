...
class GPT(nn.Module):
    def __init__(
        ...
        if share_body:
            ...
            self.shared_body = _make_block(include_controls=False)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.share_body:
            ...
            for block in self.stem_blocks:
                x = block(x, x0)
                skips.append(x)
            # Body: shared weights applied repeatedly with per-application controls.
            for ctrl in self.body_controls:
                x = self.shared_body(x, x0, controls=ctrl)
...
def main() -> None:
    ...
    if args.share_body:
        ...
        log0(
            ...
            f"shared_block_params={shared_params} control_params={ctrl_params} "
            f"saved_vs_unique={(num_body_apps - 1) * shared_params - ctrl_params}"
        )
